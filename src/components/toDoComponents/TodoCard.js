  ...
  return (
    <div className="todo-container">
      <div className='todo-filters'>
        <p className={filterId === "all" ? 'active' :''} onClick={handleFiltering} id='all'>All</p>
        <p className={filterId === "active" ? 'active' :''} onClick={handleFiltering} id='active'>Active</p>
        <p className={filterId === "completed" ? 'active' :''} onClick={handleFiltering} id='completed'>Completed</p>
      </div>
       ...
       <ul className="content-list">
            {props.filteredList.map(todo => (
              <li className="todo-element" key={todo.id}>
               ...
               <input type={'checkbox'}
                        checked={todo.completed}
                        onClick={() => {props.handleDeleted(todo._id); }}
                        onChange={() => { }}
                        id={`a${todo._id}`}
